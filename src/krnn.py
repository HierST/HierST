...
class MyKRNNEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()

        self.week_em = nn.Embedding(7, config.date_emb_dim)
        self.encoder = CNNKRNNEncoder(cnn_input_dim=config.day_fea_dim+config.date_emb_dim-1,
    ...
    def forward(self, input_day, g):
        x = self.add_date_embed(input_day)
        x = self.encoder(x, g['cent_n_id'])

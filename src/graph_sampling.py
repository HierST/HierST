...
class MySAINTSampler(object):
    ...
    def __init__(self, data, batch_size,
                 center_node_ids=None, sample_type='rw',
                 walk_length=2, sample_coverage=50,
                 save_dir=None, log=True):
        assert data.edge_index is not None
        assert 'node_norm' not in data
        assert 'edge_norm' not in data
        assert sample_type in ('rn', 'rw')

        self.N = N = data.num_nodes
        self.E = data.num_edges

        # determine the center node indices1
        if center_node_ids is None:
            self.center_n_ids = torch.arange(N)
        else:
            self.center_n_ids = center_node_ids
    ...
    def __sample_nodes__(self):
        ...
        all_n_id = self.center_n_ids[torch.randperm(self.center_n_ids.shape[0])]
        node_samples = []
        for s_id in range(0, self.N, self.batch_size):
            init_n_id = all_n_id[s_id:s_id+self.batch_size]  # [batch_size]

            if self.sample_type == 'rw':
                n_id = self.rw_adj.random_walk(init_n_id, self.walk_length)  # [batch_size, walk_length+1]
    ...
    def __len__(self):
        return (self.center_n_ids.shape[0] + self.batch_size-1) // self.batch_size

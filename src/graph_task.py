...
from dgcn import DGCNModel
from zgcn import ZGCNModel
from hgcn import HGCNModel
...
class RNNConfig(BaseConfig):
    def __init__(self):
        ...
        self.model_type = 'zgcn'  # choices: krnn, sandwich, nbeats, tgcn
        self.rnn_type = 'nbeats'  # choices: krnn, nbeats, used for GCN models
        self.date_emb_dim = 2
        ...
        # for gcn
        self.gcn_dim = 32
        self.gcn_type = 'gate_gcn'  # gcn or gate_gcn
        self.gcn_aggr = 'max'
        self.gcn_norm = 'none'
...
class WrapperNet(nn.Module):
    def __init__(self, config):
        ...
        elif self.config.model_type == 'tgcn':
            self.net = TGCNModel(config)
        elif self.config.model_type == 'dgcn':
            self.net = DGCNModel(config)
        elif self.config.model_type == 'zgcn':
            self.net = ZGCNModel(config)
        elif self.config.model_type == 'hgcn':
            self.net = HGCNModel(config)
        else:
            raise Exception(
...
class RNNTask(BasePytorchTask):
    ...
    def init_data(self, data_fp=None):
        ...
        # load data
        if self.config.prepro_type == 'raw':
            self.config.use_popu_norm = False  # whether the label has been normalized by some coefficients
            self.config.use_logy = True  # whether the label has been processed by log1p
            self.config.use_fea_zscore = False  # whether to normalize features
            self.config.use_adapt_norm = False
            self.config.label_fea_name = f'{self.config.label[:-7]}_rolling'
            day_inputs, day_gov_inputs, outputs, dates, nodes, self.main_feas = \
                load_data(data_fp,
            ...
            self.gov_feas = None
            self.node_popu = None
            self.fea_scaler = None
        elif self.config.prepro_type == 'c3.ai':
            self.config.use_popu_norm = True
            self.config.use_logy = False
            self.config.use_fea_zscore = True
            self.config.use_adapt_norm = False
        ...
        elif self.config.prepro_type == 'adapt':
            self.config.use_popu_norm = False
            self.config.use_logy = False
            self.config.use_fea_zscore = False
            self.config.use_adapt_norm = True
            self.config.label_fea_name = f'{self.config.label[:-7]}.rolling({self.config.horizon}).sum()'
            day_inputs, day_gov_inputs, outputs, dates, nodes, \
                self.main_feas, self.gov_feas, self.node_popu, self.fea_scaler = \
                    load_new_data(data_fp, self.config, logger=self.log)
    ...
    def init_graph(self, graph_fp=None):
        ...
        assert self.config.num_nodes == len(self.node_name)
        self.config.num_edges = self.edge_weight.shape[0]
    ...
    def train_step(self, batch, batch_idx):
        ...
        if self.config.topo_loss_weight > 0 and \
            ...
            # if self.config.use_node_weight:
            #     masked_node_weight = node_weight[:, agg_mask]
            #     topo_loss = weighted_mse_loss((ym_hat+eps)/(ym+eps), (ym_hat_agg+eps)/(ym_agg+eps), masked_node_weight) + \
            #         weighted_mse_loss((ym_hat+eps)/(ym+eps), (ym_agg+eps)/(ym_hat_agg+eps), masked_node_weight)
            # else:
            # topo_loss = self.loss_func((ym_hat+eps)/(ym+eps), (ym_hat_agg+eps)/(ym_agg+eps)) + \
            #     self.loss_func((ym_hat+eps)/(ym+eps), (ym_agg+eps)/(ym_hat_agg+eps))
